"""
Vehicle Distance & Overtaking Analyzer

Calculates:
- Edge-to-edge distances between vehicles using 3D bounding boxes
- Following distances (same-lane interactions)
- Overtaking events based on relative longitudinal position changes

The analyzer operates on monocular video with precomputed calibration
and does not include any dataset-specific assets.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Vehicle Distance & Overtaking Analyzer")
    parser.add_argument("--lookup", required=True, help="Calibration lookup table (.npy)")
    ...
    generator = BBox3DGenerator(args.lookup)
    ...
    print("\nSummary:")
    print(f"  Distance measurements: {len(analyzer.distance_log)}")
    print(f"  Overtaking events: {len(analyzer.overtaking_events)}")
