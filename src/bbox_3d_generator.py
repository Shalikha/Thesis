...
def parse_args():
    parser = argparse.ArgumentParser(description="3D Bounding Box Generator")
    parser.add_argument(
        "--lookup",
        required=True,
        help="Path to calibration lookup table (.npy), e.g. data/calibration_lookup.npy",
    )
    parser.add_argument(
        "--image",
        required=True,
        help="Path to input image, e.g. data/sample_image.png",
    )
    parser.add_argument(
        "--outdir",
        default="output",
    ...
    parser.add_argument(
        "--model",
        default="yolov8n.pt",
        help="Ultralytics YOLO model path/name (default: yolov8n.pt)",
    )
    parser.add_argument(
        "--conf",
        type=float,
    ...
    parser.add_argument(
        "--show-2d",
        action="store_true",
        help="Also draw 2D bounding boxes",
    )
    parser.add_argument(
        "--no-info",
        action="store_true",
        help="Disable info text overlay",
    )
...
def main():
    ...
    generator = BBox3DGenerator(args.lookup, yolo_model=args.model)
    ...
    if generator.model is None:
        print("YOLO is not available (ultralytics not installed).")
        print("Install with: pip install ultralytics")
        return

    bboxes_3d = generator.process_image(image, conf_threshold=args.conf)

    if not bboxes_3d:
        print("No 3D bounding boxes generated.")
        print("\nDONE")
        return
    ...
    output_path = os.path.join(args.outdir, "3d_bboxes_result.png")
    cv.imwrite(output_path, output)
    print(f"\nSaved visualization to: {output_path}")

    kitti_path = os.path.join(args.outdir, "detections_kitti.txt")
    with open(kitti_path, "w") as f:
        f.write(generator.to_kitti_format(bboxes_3d))
    print(f"Saved KITTI output to: {kitti_path}")
