...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Interactive 3D Bounding Box Adjustment Tool"
    )
    parser.add_argument(
        "--image",
        type=str,
        required=True,
        help="Path to the image frame (e.g. sample_image.png)"
    )
    parser.add_argument(
        "--lookup",
        type=str,
        required=True,
        help="Path to calibration lookup table (.npy)"
    )
...
def main():
    ...
    if not os.path.exists(args.lookup):
        raise FileNotFoundError(f"Lookup table not found: {args.lookup}")

    tool = Interactive3DBBoxTool(
